...
export function ItemMedia({ item }: { item: CartItem }) {
  if (item.type === "bebida") return <DrinkIllus id={item.key} label={item.name} />;
  if (item.type === "empanadas") return <EmpanadaIllus id={item.key} />;
  return <PizzaIllus id={item.illus || item.key} />;
}
